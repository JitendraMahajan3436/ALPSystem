...
{
    /*               Controller Name           : PPCPlanningController
     ...
     */
    public class PPCPlanningController : BaseController
    {
        ...
        */
        public ActionResult Index()
        {
            ...
            globalData.pageTitle = ResourceModules.OM_PPC_Planning;
            globalData.subTitle = ResourceGlobal.Lists;
            globalData.controllerName = "PPCPlanning";
            globalData.actionName = ResourceGlobal.Lists;
            globalData.contentTitle = ResourceModules.OM_PPC_Planning;
            globalData.contentFooter = ResourceModules.OM_PPC_Planning;
            ...
            ViewBag.Shift = new SelectList(db.RS_Shift.Where(m => m.Shop_ID == plantid), "Shift_ID", "Shift_Name");
        ...
        *               Description               : Action used to return the list of Line ID for PPCPlanning
        *               Author, Timestamp         : Jitendra Mahajan
       ...
       */
        ...
        */
        ...
        public ActionResult loadOrderSequence(FormCollection frmFieldsArr)
        {
            ...
            orderSequenceObjSheduled = (from or in db.RS_OM_OrderRelease.Where(or => or.Plant_ID == plantid && or.Shop_ID == shopID && or.Line_ID == lineID && or.Order_Status == "Release" && or.Order_Start == false && or.Planned_Shift_ID == ShiftID)
                                        ...
                                        select new CummulativeFields()
                                        {
                                            ...
                                            Locked = (db.RS_OM_U321_Tactsheet_Orders.Any(m => m.Plant_ID == plantid && m.Shop_ID == shopID && m.Order_No == or.Order_No && m.Is_Locked == true) || db.RS_OM_S201_Tactsheet_Orders.Any(m => m.Plant_ID == plantid && m.Shop_ID == shopID && m.Order_No == or.Order_No && m.Is_Locked == true)),
                                        }).ToList();
            orderSequenceObjReleased = (from or in db.RS_OM_OrderRelease.Where(or => or.Plant_ID == plantid && or.Shop_ID == shopID && or.Line_ID == lineID && or.Order_Status == "Release" && or.Order_Start == false && (or.Planned_Shift_ID ==null || or.Planned_Shift_ID==0))
                                        orderby or.RSN ascending
                                        orderby or.Planned_Shift_ID descending
                                        select new CummulativeFields()
                                        {
                                            Row_ID = or.Row_ID,
                                            Order_No = or.Order_No,
        ...
        public ActionResult loadOrderSequenceTactsheet(FormCollection frmFieldsArr)
        {
            ...
            {
                generalHelper.addControllerException(exp, "PPCPlanningController", "loadOrderSequence(Post)", ((FDSession)this.Session["FDSession"]).userId);
            }
            finally
            {
        ...
        public ActionResult checkifUnHoldLock(decimal PlatformId)
        {
            ...
            {
                generalHelper.addControllerException(exp, "PPCPlanningController", "checkifUnHoldLock(lineID: " + lineID + ")", ((FDSession)this.Session["FDSession"]).userId);
                jsondataObj.result = true;
        ...
        public ActionResult UnLockUnholdTable(int? PlatformId)
        {
            ...
            {
                generalHelper.addControllerException(exp, "PPCPlanningController", "UnLockUnholdTable(: " + PlatformId + ")", ((FDSession)this.Session["FDSession"]).userId);
        ...
        public ActionResult saveOrderSequence(List<int> sequenceList, decimal plantID, decimal shopID, decimal PlatformId, List<string> remarks, int ShiftProdCount, int ShiftID, List<int> SelectedSequenceArr)
        {
            ...
            try
            {
                ...
                else if (db.RS_OM_Platform.Find(PlatformId).Platform_Name.Equals("S201", StringComparison.CurrentCultureIgnoreCase))
                {
                    ...
                    var OthershiftlockedOrders = (from or in db.RS_OM_OrderRelease
                                                  join tact in db.RS_OM_S201_Tactsheet_Orders on or.Order_No.ToUpper() equals tact.Order_No
                                                  where or.Order_Status == "Release" && or.Order_Start == false && tact.Plant_ID == plantID && tact.Shop_ID == shopID
                                                  && tact.Planned_Date.Year == dateVal.Year && tact.Planned_Date.Month == dateVal.Month && tact.Planned_Date.Day == dateVal.Day
                                                  && tact.Is_Locked == true && tact.Shift_ID != ShiftID
                    ...
                    foreach (var or in OthershiftlockedOrders)
                    {
                    ...
                    {
                        //errorFlag = false;
                        if ((shiftlockedOrders[s].Row_ID != sequenceList[s]) || (ShiftProdCount < shiftlockedOrders.Count() && shiftlockedOrders.Count() > 0))
                        {

                            jsonObj.type = "lockedOrder";
                            jsonObj.message = "Cant Not Save Order Sequnece, Top " + shiftlockedOrders.Count() + " Order are locked";
                            return Json(jsonObj, JsonRequestBehavior.AllowGet);
                        }
                        else if ((shiftlockedOrders[s].Shift_ID != ShiftID))
                        {
            ...
            catch (Exception exp)
            {
                ...
                generalHelper.addControllerException(exp, "PPCPlanningController", "saveOrderSequence(Post)", ((FDSession)this.Session["FDSession"]).userId);
            }
            finally
            {
        ...
        public ActionResult DeleteTractorOrder(int orderRowid, string remark)
        {
            ...
            catch (Exception exp)
            {
                ...
                generalHelper.addControllerException(exp, "PPCPlanningController", "DeleteTractorOrder()", ((FDSession)this.Session["FDSession"]).userId);
            }
            return Json(jsondataObj, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult DeleteOrder(int orderRowid, string remark)
        {
            ...
            catch (Exception exp)
            {
                ...
                generalHelper.addControllerException(exp, "PPCPlanningController", "DeleteOrder()", ((FDSession)this.Session["FDSession"]).userId);
            }
            return Json(jsondataObj, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult HoldTractorOrder(int orderRowid, string remark)
        {
            ...
            catch (Exception exp)
            {
                ...
                generalHelper.addControllerException(exp, "PPCPlanningController", "HoldTractorOrder()", ((FDSession)this.Session["FDSession"]).userId);
            }
            return Json(jsondataObj, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult getBlockUnBlockData(decimal plantId, decimal shopId, int? PlatformId)
        {
            ...
            catch (Exception exp)
            {
                ...
                generalHelper.addControllerException(exp, "PPCPlanningController", "getBlockUnBlockData()", ((FDSession)this.Session["FDSession"]).userId);
            }
            return Json(jsondataObj, JsonRequestBehavior.AllowGet);
        }
        public ActionResult UnBlockOrders(decimal shopID, decimal PlatformId)
        {
            ...
            catch (Exception exp)
            {
                jsondataObj.result = false;
                jsondataObj.message = "Exception: " + exp.Message + " | Cannot get Block/UnBlock Data .";
                generalHelper.addControllerException(exp, "PPCPlanningController", "UnBlockOrders()", ((FDSession)this.Session["FDSession"]).userId);
            }
            finally
            {
                generalHelper.logUserActivity(shopID, lineID, moduleName, activityRemarks, nowTime, DateTime.Now, ((FDSession)this.Session["FDSession"]).userId, ((FDSession)this.Session["FDSession"]).userHost);
        ...
        public ActionResult changeDailyPlan(List<string> selectedSequence, decimal shopID, decimal PlatformId, List<string> allShift, int prodCount)
        {
            ...
            int tact_time = 0;
            var shiftName = "";
            //var plannedDateVal = PlannedDate;//objRS_OM_Order_Pattern[0].Planned_Date.ToString("yyyy-MM-dd");
            //var plannedDateVAl = DateTime.Now.Date;
            ...
            //    objJSONData.message = "Exception orrcured whil saving sequence";
            ...
            if (prodCount > 0)
            {
                foreach (var id in rowIdObj)
                {
                    ...
                    //var c = db.RS_OM_OrderRelease.Where(m => m.Plant_ID == plantId && m.Shop_ID == shopID && m.Line_ID == LineID && m.Planned_Date == dateVal && m.Row_ID == id && m.Order_Type.ToLower() != "spare" && m.Order_Status.ToLower() == "release").Count();
                    if (c != 1)
                    {
                        jsondataObj.result = false;
                ...
                ////var availableproductioncount = db.RS_OM_OrderRelease.Where(m => m.Plant_ID == plantId && m.Shop_ID == shopID && m.Line_ID == LineID && m.Planned_Date == dateVal && m.Order_Type.ToLower() != "spare" && m.Order_Status.ToLower() == "release").Count();
                ////if (!(rowIdObj.Count() <= availableproductioncount))
                ////{
                ////    //objJSONData.status = false;
                ...
                ////    jsondataObj.result = false;
                ////    jsondataObj.message = "Production count does not matches as per planned date... You have to create Order!...or Chnage Prduction Count";
                ...
                //var availableproductioncount = db.RS_OM_OrderRelease.Where(m => m.Plant_ID == plantId && m.Shop_ID == shopID && m.Line_ID == LineID && m.Planned_Date == dateVal && m.Order_Type.ToLower() != "spare" && m.Order_Status.ToLower() == "release" && m.Planned_Shift_ID == null).Count();
                //if (!(prodCount <= availableproductioncount))
                //{
                //    //objJSONData.status = false;
                //    //objJSONData.message = "Production count does not matches as per planned date... You have to create Order!...or Chnage Prduction Count";
                //    //objJSONData.type = "fail";
            ...
            try
            {
                if (objShift != null)
                {
                    ...
                    //add planned shift to each orders
                    //List<RS_OM_OrderRelease> orders = new List<RS_OM_OrderRelease>();
                    //foreach (var id in rowIdObj)
                    //{

                    //    orders.Add(db.RS_OM_OrderRelease.Find(id));
                    //}
                    ...
                    //var orders = db.RS_OM_OrderRelease.Where(m => m.Plant_ID == plantId && m.Shop_ID == shopID && m.Line_ID == LineID && m.Order_Status == "Release" && m.Order_Type.ToLower() != "spare" && m.Order_Start == false && DbFunctions.TruncateTime(m.Planned_Date) == DbFunctions.TruncateTime(DateTime.Now)).OrderBy(m => m.RSN).ToList();
                    //int m = 0;
                    //int j = 0;
                    //int k = 0;
            ...
            catch (Exception exp)
            {
                ...
                generalHelper.addControllerException(exp, "PPCPlanningController", "changeDailyPlan()", ((FDSession)this.Session["FDSession"]).userId);
            }
            finally
            {
        ...
        private void updatePlannedOrders(decimal shopID)
        {
            ...
            catch (Exception exp)
            {
                ...
                generalHelper.addControllerException(exp, "PPCPlanningController", "updatePlannedOrders(Shop_ID:" + shopID + ")", ((FDSession)this.Session["FDSession"]).userId);
            }
            ...
            catch (System.Data.Entity.Validation.DbEntityValidationException dbex)
            {
                ...
                foreach (var ValidationErrors in dbex.EntityValidationErrors)
                {
                    foreach (var validationError in ValidationErrors.ValidationErrors)
                    {
                        ...
                        generalHelper.addControllerException(dbex, "PPCPlanningController", "SaveORderPattern(Post) " + validationError, ((FDSession)this.Session["FDSession"]).userId);
                    }
                }
                throw raise;
            }
        ...
        */
        public ActionResult GenerateTactSheet(List<string> allShift, decimal platform_Id, int ShopID, int? prodCount)
        {
            ...
            try
            {
                ...
                //var plannedDateVal = PlannedDate;//objRS_OM_Order_Pattern[0].Planned_Date.ToString("yyyy-MM-dd");
                //var plannedDateVal = PlannedDate;
                ...
                if (prodCount > 0)
                {
                    ...
                    neworderPatternSequenceObj = (from or in neworderPatternSequenceObj.Where(m => m.orderType.ToLower() != "spare" && m.Planned_Shift_ID == shiftId)
                    ...
                    if (neworderPatternSequenceObj != null)
                    {
                        ...
                        var U321RSNCount = db.RS_OM_U321_Tactsheet_Orders.Where(m => m.Is_Locked == true &&
                          m.Plant_ID == plantID && m.Shop_ID == ShopID && m.Planned_Date.Year == plannedDate.Year
                               && m.Planned_Date.Month == plannedDate.Month && m.Planned_Date.Day == plannedDate.Day).Max(q => (int?)(q.RSN)) ?? 0;

                        var U321lockSkipCount = db.RS_OM_U321_Tactsheet_Orders.Where(m => m.Is_Locked == true && m.Order_Status == 1 && m.Is_Started != true &&
                         m.Plant_ID == plantID && m.Shop_ID == ShopID && m.Planned_Date.Year == plannedDate.Year
                              && m.Planned_Date.Month == plannedDate.Month && m.Planned_Date.Day == plannedDate.Day).Count();
                        ...
                        var S201RSNCount = db.RS_OM_S201_Tactsheet_Orders.Where(m => m.Is_Locked == true &&
                         m.Plant_ID == plantID && m.Shop_ID == ShopID && m.Planned_Date.Year == plannedDate.Year
                               && m.Planned_Date.Month == plannedDate.Month && m.Planned_Date.Day == plannedDate.Day).Max(q => (int?)(q.RSN)) ?? 0;
                        var S201lockSkipCount = db.RS_OM_S201_Tactsheet_Orders.Where(m => m.Is_Locked == true && m.Order_Status == 1 && m.Is_Started != true &&
                        m.Plant_ID == plantID && m.Shop_ID == ShopID && m.Planned_Date.Year == plannedDate.Year
                              && m.Planned_Date.Month == plannedDate.Month && m.Planned_Date.Day == plannedDate.Day).Count();
                        //S201lockSkipCount++;
                        ...
                        else
                        {
                            ...
                            U321RSNSkipCount = db.RS_OM_U321_Tactsheet_Orders.Where(m => m.Is_Locked == true  && m.Is_Started != true && m.Plant_ID == plantID && m.Shop_ID == ShopID && m.Order_Status == 1).Count();
                            U321RSNCount++;
                        ...
                        else
                        {
                            ...
                            S201RSNSkipCount = db.RS_OM_S201_Tactsheet_Orders.Where(m => m.Is_Locked == true && m.Is_Started != true && m.Plant_ID == plantID && m.Shop_ID == ShopID && m.Order_Status == 1).Count();
                            S201RSNCount++;
                        }
                        var bot = "";
                        ...
                        {
                            //delete already scheduled order in same shift
                            var oldTactsheetOdr = db.RS_OM_S201_Tactsheet_Orders.Where(m => m.Plant_ID == plantID && m.Shop_ID == ShopID  && m.Is_Locked != true && m.Shift_ID == shiftId).ToList();
                                    ...
                                    for (int k = 0; k < productionCount; k++)
                                    {
                                        ...
                                        if (firstBreakTime != null)
                                        {
                                            ...
                                            if (t2.Minutes == 0 && t2.Seconds == 0)
                                            {
                                                ...
                                                dt.Rows.Add(rownumber, neworderPatternSequenceObj[k].Order_No, neworderPatternSequenceObj[k].Auto_Remarks, neworderPatternSequenceObj[k].Color_Name, drive, startdisplayTime, displayTime);
                                                rownumber++;
                                        ...
                                        if (SecondBreakTime != null)
                                        {
                                            ...
                                            if (t2.Minutes == 0 && t2.Seconds == 0)
                                            {
                                                ...
                                                timespan = DateTime.Today.Add(t1);
                                                displayTime = timespan.ToString("hh:mm:ss tt");
                                                //dt.Rows.Add(rownumber++, objRS_OM_Order_Pattern[j].Platform_Name, objRS_OM_Order_Pattern[j].ModelCode, objRS_OM_Order_Pattern[j].Color, drive, displayTime);
                                                dt.Rows.Add(rownumber, neworderPatternSequenceObj[k].Order_No, neworderPatternSequenceObj[k].Auto_Remarks, neworderPatternSequenceObj[k].Color_Name, drive, startdisplayTime, displayTime);
                                                rownumber++;
                                            }
                                            else
            ...
            catch (System.Data.Entity.Validation.DbEntityValidationException dbex)
            {
                ...
                foreach (var ValidationErrors in dbex.EntityValidationErrors)
                {
                    foreach (var validationError in ValidationErrors.ValidationErrors)
                    {
                        ...
                        generalHelper.addControllerException(dbex, "PPCPlanningController", "GenerateTactSheet(Post) " + validationError, ((FDSession)this.Session["FDSession"]).userId);
                    }
                }
                throw raise;
            }
            catch (Exception ex)
            {
                ...
                objJSONData.type = "fail";
                generalHelper.addControllerException(ex, "PPCPlanningController", "GenerateTactSheet(Post)", ((FDSession)this.Session["FDSession"]).userId);
                return Json(objJSONData, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult GenerateAndDownloadTactSheet(List<string> allShift, decimal platform_Id, int ShopID)
        {
            ...
            catch (System.Data.Entity.Validation.DbEntityValidationException dbex)
            {
                ...
                foreach (var ValidationErrors in dbex.EntityValidationErrors)
                {
                    foreach (var validationError in ValidationErrors.ValidationErrors)
                    {
                        string Message = string.Format("{0:}{1}", ValidationErrors.Entry.Entity.ToString(), validationError.ErrorMessage);
                        raise = new InvalidOperationException(Message, raise);
                        generalHelper.addControllerException(dbex, "PPCPlanningController", "GenerateAndDownoadTactSheet(Post) " + validationError, ((FDSession)this.Session["FDSession"]).userId);
                    }
                }
                throw raise;
            }
            catch (Exception ex)
            {
                ...
                objJSONData.message = ex.Message;
                objJSONData.type = "fail";
                generalHelper.addControllerException(ex, "PPCPlanningController", "GenerateAndDownoadTactSheet(Post)", ((FDSession)this.Session["FDSession"]).userId);
                return Json(objJSONData, JsonRequestBehavior.AllowGet);
        ...
        public Boolean updateOrderStartStatus(int shopId)
        {
           ...
           && today.Day == m.Planned_Date.Day).OrderBy(m => m.RSN).ToList();
            ...
            catch (Exception ex)
            {
                ...
                generalHelper.addControllerException(ex, "PPCPlanningController", "updateOrderStartStatus()", ((FDSession)this.Session["FDSession"]).userId);
                return false;
     ...
     && today.Day == m.Planned_Date.Day).OrderBy(m => m.RSN).ToList();
        ...
        */
        public string ExportToPdf(DataSet ds, string platformName, string shiftName, int tact_time)
        {
            ...
            Phrase cell = new Phrase(platformName + " TactSheet                                       Tact Time: " + Tact_Time + "\n", tactsheetfont);
