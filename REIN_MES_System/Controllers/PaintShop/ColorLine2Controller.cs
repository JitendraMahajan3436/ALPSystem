...
{
    public class ColorLine2Controller : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            globalData.pageTitle = "Colour Line 2";
        ...
        public ActionResult ValidateSerialNo(string SerialNo, bool WIP, bool CompletedRecord)
        {
            ...
            else
            {
                ...
                {
                    var currentStatus = db.RS_OM_Order_Status_Live.Where(m => m.Serial_No == SerialNo && (m.Action_Status == "Colour Line 2")).ToList();
                    if (currentStatus.Count() > 0)
                        return Json(new { Result = true, Msg = "" }, JsonRequestBehavior.AllowGet);
                    else
                        return Json(new { Result = false, Msg = "Current Body Status is not Colour Line 2" }, JsonRequestBehavior.AllowGet);
                }
            }
        }
        ...
        public ActionResult GetWIPData(string Serial_No = "")
        {
            ...
            try
            {
                if (Serial_No == "" || Serial_No.Length > 5)
                {
                    ...
                    var result1 = db.RS_OM_Order_Status_Live.Where(m => m.Action_Status == "Colour Line 2").OrderByDescending(m => m.Updated_Date).Select(m => m.Serial_No).FirstOrDefault();

                    var CompletedCount = (from s in db.RS_OM_Order_Status_Live
                                          where s.Action_Status == "Colour Line 2"
                                          select s).Count();
        ...
        public ActionResult GetCompletedData()
        {
            ...
            try
            {
                ...
                var result = (from s in db.RS_OM_Order_Status_Live.Where(m => m.Action_Status == "Colour Line 2")
                              join c in db.RS_OM_Order_List on s.Serial_No equals c.Serial_No
                              join d in db.RS_Model_Master on c.Model_Code equals d.Model_Code
        ...
        public ActionResult SaveData(string SerialNo, bool WIP, bool CompletedRecord, string Skid_No, string Hanger_No, string Barcode, int Shop_Id)
        {
            ...
            try
            {
                ...
                if(WIP == true)
                {
                    if (!string.IsNullOrEmpty(orderNo))
                    {
                        ...
                        if (ExistingOrder != null)
                        {
                            ...
                            ExistingOrder.Action_Status = "Colour Line 2";
                        ...
                        else
                        {
                            ...
                            ValidMsg = "Body ColorLine2 Successfully";
                        }
                    }
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(orderNo))
                        {
                            ...
                            objOrderRelease.Order_Status = "Colour Line 2";
                    ...
                    {
                        return Json(new { Result = false, Msg = "Error while changing Order Status to Colour Line 2" }, JsonRequestBehavior.AllowGet);
                    }
                ...
                else
                {
                    ...
                    if (ExistingOrder != null && CurrentStatus == "Colour Line 2")
                    {
