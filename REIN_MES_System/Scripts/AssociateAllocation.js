$(document).ready(function (e) {
    ...
    $(".manager_Line #Station_ID").change(function (e) {
        ...
        if (stationId) {
            ...
            var jsonData = JSON.stringify({ plantId: 7 });
            url = "/ShopManagerConfiguration/GetSkilledOperatorByStationID";
            ajaxpack.getAjaxRequest(url, "stationId=" + $("#Station_ID").val() + "", showStationSkilledOperatorType, "json");
        }

    });


    $(".manager_Line #Operator_ID").change(function (e) {
        //alert("Hi nil");
        // var jsonData = JSON.stringify({ plantId: 7 });
        ...
        if (operatorId) {
            ...
            url = "/ShopManagerConfiguration/GetOperatorSkillSetByOperatorID";
            ajaxpack.getAjaxRequest(url, "operatorId=" + $("#Operator_ID").val() + "", showOperatorSkillSetType, "json");

        }
    });


    function showOperatorSkillSetType() {
        ...
        if (myajax.readyState == 4) {
            if (myajax.status == 200 || window.location.href.indexOf("http") == -1) { //if request was successful or running script locally
                ...
                SelectOptionShiftHTML(jsonRes, "lblSkillSet");
            }
        }
    }
    ...
    function showStationSkilledOperatorType() {
        var myajax = ajaxpack.ajaxobj
        var myfiletype = ajaxpack.filetype
        if (myajax.readyState == 4) {
            if (myajax.status == 200 || window.location.href.indexOf("http") == -1) { //if request was successful or running script locally
